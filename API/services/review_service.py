...
class ReviewService:
    ...
    async def get_reviews():
        try:
            response = supabase_client.table(REVIEWS_TABLE).select("*").order("created_at", desc=True).execute()
    ...
    async def get_reviews_by_match(match_id: int):
        try:
            response = supabase_client.table(REVIEWS_TABLE).select("*").eq("match_id", match_id).order("created_at", desc=True).execute()
            return response.data
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error: {str(e)}")
