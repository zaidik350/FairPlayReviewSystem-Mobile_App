...
import type { CreateMatchRequest, UpdateMatchRequest } from '@/types/api.types';
...
class MatchService {
  ...
  async getAll(): Promise<Match[]> {
    if (API_CONFIG.USE_REAL_API) {
      const res = await apiClient.get<Match[]>(API_ENDPOINTS.MATCHES.LIST);
      return res.data;
    }
  ...
  async create(data: CreateMatchRequest): Promise<Match> {
    if (API_CONFIG.USE_REAL_API) {
      const res = await apiClient.post<Match>(API_ENDPOINTS.MATCHES.CREATE, data);
      return res.data;
    }
    const newMatch: Match = { ...data, id: Date.now().toString(), status: data.status || 'upcoming' };
